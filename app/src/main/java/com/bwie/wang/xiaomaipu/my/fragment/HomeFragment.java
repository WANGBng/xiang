...
import com.bwie.wang.xiaomaipu.my.activity.details.ViewPagerAdapter;
...
public class HomeFragment extends Fragment implements HomeView, NavView {
    ...
    @BindView(R.id.related_all)
    RelativeLayout relatedAll;
...
//                }
     ...
     */
    @Override
    public void onHomeCommoditySuccess(final HomeCommodityBean.ResultBean homeCommodityBean) {
...
//      魔力时尚
        ...
        moRecyclerView.addItemDecoration(new SpacesItemDecoration(10));
//
//      品质生活
...
//热销
        home_hot_line_adapter.setOnHomeHotLineClickLister(new Home_Hot_Line_Adapter.OnHomeHotLineAdapterClickLister() {
            ...
            public void onHomeHotLineAdapterItemClickLister(int position) {
                ...
                relatedAll.setVisibility(View.GONE);
                homeDetails.setVisibility(View.VISIBLE);
/**
 *             int commodityId = homeCommodityBean.getRxxp().get(position).getCommodityList().get(position).getCommodityId();
 */
            }
        });
//        魔力时尚
...
//        进行获取焦点
    ...
    private void getFourse() {
        ...
        getView().setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
                    relatedAll.setVisibility(View.VISIBLE);
